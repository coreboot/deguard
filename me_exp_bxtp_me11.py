...
# Bootguard disable support added by Mate Kukri
...
def GenerateRops(me_info):
    ...
    # rops += rop(write_selector)
    # rops += rop(infinite_loop)
    # rops += rop(dfx_agg_sel)
    ...
    ### START - bootguard disable rops

    def gen_write_rop(rops, addr, val):
    ...
    rops = gen_write_rop(rops, 0x85274 + 0x14, 0x04400044)
    rops = gen_write_rop(rops, 0x85274 + 0xa0, 0x02fe3d71)

    ### END - bootguard disable rops

    # NOTE: uncomment this for red unlock
    # Write dfx personality = 0x3
