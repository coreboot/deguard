...
class CFGRecord(object):
  ...
  def copy(self):
    return self.createRecord(self.name, self.mode, self.opt,
                             self.uid, self.gid, self.size, self.offset)

  def __str__(self):
    return "%-12s (%04X:%04X) [%4d bytes @ %8X] %s _ %s" % (self.name, self.uid, self.gid,
                                                            self.size, self.offset,
                                                            CFG.modeToStr(self.mode),
                                                            CFG.optToStr(self.opt))
...
class CFGFile(object):
  def __init__(self, path, record, data, parent=None):
    ...
    self.parent = parent
    self.children = []
    #print "New %-10s : %-32s with parent %s" % ("directory" if self.isDirectory() else "file", self.path, self.parent.path if parent else "NONE")
    if parent:
